...
/*  Created On : 05-dic.-2016 20:47:44 				*/
...
IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[FK_conversions_warehouses]') AND OBJECTPROPERTY(id, N'IsForeignKey') = 1) 
ALTER TABLE [conversions] DROP CONSTRAINT [FK_conversions_warehouses]
GO
...
IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[FK_dishes_category]') AND OBJECTPROPERTY(id, N'IsForeignKey') = 1) 
ALTER TABLE [dishes] DROP CONSTRAINT [FK_dishes_category]
GO
...
(
	...
	[emp_passwd] varchar(40) NOT NULL
)
GO
...
(
	...
	[ord_dis_date] datetime NOT NULL,
	[ord_dis_subtotal] money NOT NULL
)
GO
...
ALTER TABLE [conversions] 
 ADD CONSTRAINT [CH_conversion_factor] CHECK (cvn_factor_number > 0)
GO
...
ALTER TABLE [dish_ingredients] 
 ADD CONSTRAINT [CH_dis_ing_quantity] CHECK (dis_ing_quantity > 0)
GO
...
ALTER TABLE [dishes] 
 ADD CONSTRAINT [CH_dis_price] CHECK (dis_price > 0)
GO
...
ALTER TABLE [movements] 
 ADD CONSTRAINT [CH_movements_quantity] CHECK (mov_quantity > 0)
GO
...
ALTER TABLE [order_dishes] 
 ADD CONSTRAINT [CH_ord_dis_quantity] CHECK (ord_dis_quantity > 0)
GO
...
CREATE NONCLUSTERED INDEX [CH_ord_dis] 
 ON [order_dishes] ()
GO
...
ALTER TABLE [orders] 
 ADD CONSTRAINT [CH_ord_total] CHECK (ord_total >= 0)
GO

ALTER TABLE [orders] 
 ADD CONSTRAINT [CH_ord_subtotal] CHECK (ord_subtotal >= 0)
GO

ALTER TABLE [orders] 
 ADD CONSTRAINT [CH_ord_iva] CHECK (ord_iva >= 0)
GO

ALTER TABLE [orders] 
 ADD CONSTRAINT [CH_ord_status] CHECK (ord_status >= 0 AND ord_status < 3)
GO
...
ALTER TABLE [stock] 
 ADD CONSTRAINT [CH_stock_quantity] CHECK (sto_quantity >= 0)
GO

ALTER TABLE [stock] 
 ADD CONSTRAINT [CH_stock_quantity_max] CHECK (sto_min >= 0)
GO

ALTER TABLE [stock] 
 ADD CONSTRAINT [CH_stock_quantity_min] CHECK (sto_max >= 0)
GO
...
ALTER TABLE [conversions] ADD CONSTRAINT [FK_conversions_warehouses]
	FOREIGN KEY ([cvn_to_warehouse]) REFERENCES [warehouses] ([war_id]) ON DELETE No Action ON UPDATE No Action
GO
